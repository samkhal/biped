...
% max power in W
% average power in W
function power= energy_plot(trajectory)
...
powerSaved = zeros(size(keys,1), size(trajectory.torque,2));
...
for i = 1:size(keys,1)
    ...
    vels = [trajectory.vel.(joint)];
    torques = [trajectory.torque.(joint)];
    savedTorques = 
    for j = 1:length([trajectory.vel.(joint)])
        power(i,j) = electrical_power([motor_values(2), motor_values(3)],vels(j).*gearbox,torques(j).*(1/gearbox), 0);
        powerSaved(i,j) = electrical_power([motor_values(2), motor_values(3)],vels(j).*gearbox,torques(j).*(1/gearbox), 0);
    end
end
end