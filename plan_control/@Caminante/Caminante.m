classdef Caminante < TimeSteppingRigidBodyManipulator & Biped
  methods
    ...
    function obj=Caminante(urdf,options)
      ...
      if ~isfield(options,'caminante_version') 
        options.caminante_version = 5; 
      end
      ...
      if options.caminante_version == 3
      ...
      if isfield(options, 'caminante_version')
        obj.caminante_version = options.caminante_version;
      end
    ...
    function obj = compile(obj)
      ...
      % larger state frame
      if (strcmp(obj.manip.getStateFrame().getFrameByNum(1).name, 'caminantePosition'))
      ...
      if (obj.hand_right > 0)
        id = caminante_state_frame.getFrameNumByName('s-model_articulatedPosition+s-model_articulatedVelocity');
        if (length(id) > 1)
          id = id(1);
        end
        caminante_state_frame = replaceFrameNum(caminante_state_frame,id,caminanteFrames.HandState(obj,id,'right_caminanteFrames.HandState'));
      end
      if (obj.hand_left > 0)
        id = caminante_state_frame.getFrameNumByName('s-model_articulatedPosition+s-model_articulatedVelocity');
        if (length(id) > 1)
        ...
        caminante_state_frame = replaceFrameNum(caminante_state_frame,id,caminanteFrames.HandState(obj,id,'left_caminanteFrames.HandState'));
      end
      ...
      if (obj.hand_right > 0)
        ...
        input_frame = replaceFrameNum(input_frame,id,caminanteFrames.HandInput(obj,id,'right_caminanteFrames.HandInput'));
      end
      if (obj.hand_left > 0)
        id = input_frame.getFrameNumByName('s-model_articulatedInput');
        if (length(id) > 1)
        ...
        input_frame = replaceFrameNum(input_frame,id,caminanteFrames.HandInput(obj,id,'left_caminanteFrames.HandInput'));
      end
  ...
  properties
    fixed_point_file = 'data/caminante_walking_fp.mat';
    default_footstep_params = struct('nom_forward_step', 0.25,... % m
                                      ...
                                      'nom_upward_step', 0.24,... % m
                                      'nom_downward_step', 0.24,...% m
                                      'max_num_steps', 10,...
                                      'min_num_steps', 1,...
    ...
    default_walking_params = struct('step_speed', 0.5,... % speed of the swing foot (m/s)
                                    ...
                                    'pelvis_height_above_foot_sole', 0.57,... % default pelvis height when walking
