...
% Inputs:
% tau_profile, function handle to angle->torque mapping
...
r0 = tau_profile(thetas(end))/spring_max; % radius at maximum force
...
for i = 2:length(thetas)
    optim_fun = @(alpha_dist) abs(tau_profile(thetas(end-i+1)) - next_torque(i, alpha_dist));
