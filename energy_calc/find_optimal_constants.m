...
function cost = cost_function(x)
    power = zeros(2,length(states));
    additional_torque(:,:) = foo(states(joints_angle(:),:), x);
    %for each motor, for each time, add up energy with and without spring 
    tic
    for i = 1:2
            power(i,:) = electrical_power([motor_values(2), motor_values(3)],gearbox.*states(joints_speed(i),:), 1/gearbox.*abs(torques(joints_torque(i),:)+additional_torque(i,:)), 0);
    end
